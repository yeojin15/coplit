...
function studentReports(students) {
  ...
  let femaleData = students.filter((el)=>{
  ...
  //   let total = el['grades'].reduce((acc, cur)=>{
  //     return acc+cur
  //   },0);
  //   let avg = total / el['grades'].length;
  //   el['grades'] = avg;
  //   return el;
  // })
}
...
function studentReports(students) {
  // TODO: 여기에 코드를 작성합니다.
  const onlyFemale = students.filter((el)=>{
    return el['gender'] === 'female';
  });
  ...
  return onlyFemale.map((el)=>{
    ...
    const average = sum/el.grades.length;
  ...
  객체의 키:밸류
  el.name = onlyFemale.name,
  el.gender = 'female',
  el.grades = onlyFemale.averGrade
*/
